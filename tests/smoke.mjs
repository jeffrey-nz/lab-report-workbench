...
/** Text nodes anywhere on the page that read as a programming mistake. */
const placeholders = async () => page.evaluate(() => {
  const found = [];
  const walk = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
  for (let n; (n = walk.nextNode());) {
    if (/\b(null|undefined|NaN|\[object Object\])\b/.test(n.nodeValue))
      found.push(`"${n.nodeValue.trim().slice(0, 60)}" in .${n.parentElement.className || n.parentElement.tagName}`);
  }
  document.querySelectorAll("svg text").forEach((t) => {
    if (/null|undefined|NaN/.test(t.textContent)) found.push(`svg: ${t.textContent}`);
  });
  return found;
});
